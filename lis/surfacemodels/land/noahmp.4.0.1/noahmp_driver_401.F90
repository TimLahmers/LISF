...
subroutine noahmp_driver_401(n, ttile, itimestep, &  
     ...
     gla_opt, sndpth_gla_opt, chan_exfil_opt, enable2waycpl,     & ! in : new options
     rsf_opt, soil_opt, pedo_opt, crop_opt, iz0tlnd, urban_opt,  & ! in : new options
  ...
  integer, intent(in) :: chan_exfil_opt       ! MMF Channel Exfiltration (0->Default MMF Expon.; 1->LEAF model w/HyMAP) [-] 
...
! Code from module_NoahMP_hrldas_driver.F.  Initial guess only.
  ...
  call noahmplsm_401  (LIS_rc%udef,  & ! in : LIS undefined value (David Mocko)
       ...
       dveg_opt, crs_opt , btr_opt ,run_opt  , sfc_opt , frz_opt,  & ! in : user options
       inf_opt , rad_opt , alb_opt ,snf_opt  , tbot_opt, stc_opt,  & ! in : user options
       gla_opt , sndpth_gla_opt, chan_exfil_opt, enable2waycpl,     & ! in : user options
       rsf_opt , soil_opt,pedo_opt , crop_opt, iz0tlnd , urban_opt, & ! in : user options
