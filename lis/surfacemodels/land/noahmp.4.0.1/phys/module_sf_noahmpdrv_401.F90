...
! The subroutine name has been modifed for LIS implemenation Oct 22 2018
  SUBROUTINE noahmplsm_401(LIS_undef_value,                                       & ! IN : LIS undefined value
		 ...
		 P8W3D,PRECIP_IN,        SR,                                & ! IN : Forcing
                ...
                SMSTOT,SFCRUNOFF, UDRUNOFF,    ALBEDO,   QSNBOT,   SUBSNOW, & ! IN/OUT LSM eqv
                SNOWC,     SMOIS,      PAH,                                 & ! IN/OUT LSM eqv
...
!    USE MODULE_SF_NOAHMPLSM_401, only: noahmp_options, NOAHMP_SFLX, noahmp_parameters
    ...
    USE module_sf_urban_401,    only: IRI_SCHEME
!    USE module_ra_gfdleta_401,  only: cal_mon_day
!----------------------------------------------------------------
    IMPLICIT NONE
!----------------------------------------------------------------
...
!   REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(INOUT) ::  ALBEDO    ! total grid albedo []
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT) ::    ALBEDO    ! total grid albedo []
...
! Intermediate terms
    ...
    REAL,  INTENT(OUT)                  :: SUBSNOW      ! snow sublimation [mm/s]
...
! Initialized local
       ...
       IF ( VEGTYP == ISICE_TABLE ) THEN
         ...
         CALL NOAHMP_GLACIER(     I,       J,    COSZ,   NSNOW,   NSOIL,      DT, & ! IN : Time/Space/Model-related
			      ...
			      EMISSI,  FPICE,    CHB2, SUBSNOW                    & ! OUT :
#ifdef WRF_HYDRO
                              , sfcheadrt(i,j)                                      &
#endif
                              )

! David Mocko - setting glacier point undefined values to the LIS undefined value for output
         FSNO   = 1.0       
         TV     = LIS_undef_value     ! Output from standard Noah-MP undefined for glacier points
         TGB    = TG 
         CANICE = LIS_undef_value 
         CANLIQ = LIS_undef_value 
         EAH    = LIS_undef_value 
         TAH    = LIS_undef_value
         FWET   = LIS_undef_value 
         WSLAKE = LIS_undef_value 
!         ZWT    = LIS_undef_value 
         WA     = LIS_undef_value 
         WT     = LIS_undef_value 
         LFMASS = LIS_undef_value 
         RTMASS = LIS_undef_value 
         STMASS = LIS_undef_value 
         WOOD   = LIS_undef_value 
         GRAIN  = LIS_undef_value
         GDD    = LIS_undef_value
         STBLCP = LIS_undef_value 
         FASTCP = LIS_undef_value 
         PLAI   = LIS_undef_value 
         PSAI   = LIS_undef_value 
         T2MV   = LIS_undef_value 
         Q2MV   = LIS_undef_value 
         NEE    = LIS_undef_value 
         GPP    = LIS_undef_value 
         NPP    = LIS_undef_value 
         FVEGMP = 0.0 
         ...
         LAISHA = LIS_undef_value
         RB     = undefined_value
         RS(I,J)= LIS_undef_value
         BGAP   = LIS_undef_value
         WGAP   = LIS_undef_value
         TGV    = LIS_undef_value
         CHV    = LIS_undef_value 
         CHB    = CH 
         IRC    = LIS_undef_value 
         ...
         FCEV   = LIS_undef_value 
         FCTR   = LIS_undef_value        
         Z0WRF  = 0.002
         QFX(I,J) = ESOIL
         LH (I,J) = FGEV
         RELSMC = 1.0
...
! INPUT/OUTPUT
	     ...
	     SMSTAV   (I,J)                = 0.0  ! [maintained as Noah consistency]
             ...
             IF ( SALB > -999 ) THEN
                ALBEDO(I,J)                = SALB
! Added by David Mocko to output albedo as undefined when SWdown = 0
             ELSE
                ALBEDO(I,J)                = LIS_undef_value
             ENDIF
             SNOWC    (I,J)                = FSNO
             ...
             SNOW     (I,J)                = SWE
             SNOWH    (I,J)                = SNDPTH
! Added by David Mocko to ensure total canopy water output is realistic
             if ((CANLIQ.eq.LIS_undef_value).or.(CANICE.eq.LIS_undef_value)) then
                CANWAT   (I,J)                = LIS_undef_value
             else
...
SUBROUTINE PEDOTRANSFER_SR2006(nsoil,sand,clay,orgm,parameters)
  ...
  use module_sf_noahmplsm_401
  use noahmp_tables_401
  implicit none                                          ! added by Shugong Wang
  integer,                    intent(in   ) :: nsoil     ! number of soil layers
...
!jref:end       
  ...
  USE NOAHMP_TABLES_401
  use module_sf_gecros_401, only: seednc,sla0,slnmin,ffat,flig,foac,fmin,npl,seedw,eg,fcrsh,seednc,lnci,cfv
...
!!!! The following code will be called in noahmp_setup Shugong Wang 11/06/2018 
!!! SW    MMINSL='STAS'
!!! SW
!!! SW    call read_mp_veg_parameters(trim(MMINLU))
!!! SW    call read_mp_soil_parameters()
!!! SW    call read_mp_rad_parameters()
!!! SW    call read_mp_global_parameters()
!!! SW    call read_mp_crop_parameters()
!!! SW    call read_mp_optional_parameters()
...
!------------------------------------------------------------------------------------------
  ...
  USE module_sf_urban_401,    only: urban
  USE module_sf_bep_401,      only: bep
  USE module_sf_bep_bem_401,  only: bep_bem
!  USE module_ra_gfdleta_401,  only: cal_mon_day
  USE NOAHMP_TABLES_401, ONLY: ISURBAN_TABLE
  USE module_model_constants_401, only: KARMAN, CP, XLV
!----------------------------------------------------------------
    IMPLICIT NONE
!----------------------------------------------------------------
...
!------------------------------------------------------------------------------------------
!
!!! The following subroutine cal_mon_day is copied from module_ra_gfdleta, which depends on 
!!! many modules. For simplicity, only the subroutine is copied here.  Shugong Wang 10/29/2018
...
END MODULE module_sf_noahmpdrv_401
