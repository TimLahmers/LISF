...
! Root scheme variables
  ...
  INTEGER                                           :: LROOT           ! layer of root zone for local BRTRANI calculation (-)
...
!jref:end  
  ...
  REAL, PARAMETER                   :: PSILF = -150. ! leaf wilting point matric potential (m)
  REAL, PARAMETER                   :: PSIFC = -3.366 ! matric potential at field capacity (m)
...
! Local variables for root scheme
  REAL :: SMOFC, SMOMIN
...
! initialize fluxes from veg. fraction
    ...
    ! Zhuo Wang added (RB is not input or output in SUBROUTINE ENERGY) 12/02/2018
    RB        = 0.
...
! soil moisture factor controlling stomatal resistance
     ...
     IF (OPT_ROOT == 2) THEN
       ...
       END DO
       KROOT = MIN(MAX(IZ-1,1),NSOIL) ! Bottom layer of root zone
       IF (ROOT_UPDATE) THEN ! Only use modified depth when root zone is on...
         LROOT = KROOT
       ELSE
         LROOT = parameters%NROOT
       END IF
     ELSE
       LROOT = parameters%NROOT
     END IF
     ...
     IF(IST ==1 ) THEN
       DO IZ = 1, LROOT
          IF(OPT_BTR == 1) then                  ! Noah
            ...
            IF (OPT_ROOT == 2) THEN
                !print *, parameters%PSISAT(IZ) / PSIWLT
                !print *, 1. / parameters%BEXP(IZ)
                SMOMIN = parameters%SMCMAX(IZ) * ( -parameters%PSISAT(IZ) / PSIWLT )**( 1. / parameters%BEXP(IZ) )
                SMOFC = parameters%SMCMAX(IZ) * ( -parameters%PSISAT(IZ) / PSIFC )**( 1. / parameters%BEXP(IZ) )    !Field Capacity
                GX = (SH2O(IZ)-SMOMIN) / (SMOFC-SMOMIN)  !GX based on field capacity (SMOFC) and wilting point based on matric potential (SMOMIN)
            END IF
          END IF
          ...
          GX = MIN(1.,MAX(0.,GX))
          BTRANI(IZ) = MAX(MPE,DZSNSO(IZ) / (-ZSOIL(LROOT)) * GX) !TML use depth of rootzone for BTRAN
          BTRAN      = BTRAN + BTRANI(IZ)
       END DO
       BTRAN = MAX(MPE,BTRAN)

       BTRANI(1:LROOT) = BTRANI(1:LROOT)/BTRAN !TML use depth of rootzone for BTRAN


     ! Root water uptake scheme (Fan et al. 2017) ------------------------------------------------
       IF (OPT_ROOT == 2) THEN
       ! Determine layer where water table is located, if within resolved soil layers
       DZSROOT = DZSNSO(1:NSOIL) ! Soil thickness variable just for use in root scheme
       
       IF(ZWT .lt. ZSOIL(NSOIL)) THEN ! Set KWTD to NSOIL if water table below resolved layers
           KWTD = NSOIL
       ELSE
           DO IZ = 1, NSOIL ! Find exact layer with water table if within resolved layers
               if(ZWT .gt. ZSOIL(IZ))exit
           END DO
           KWTD = MIN(IZ,NSOIL) ! Layer with water table
           IWTD = KWTD-1 ! Layer above water table
           
           ! Adjust layer thickness accordingly
           IF (IWTD .ge. 1) DZSROOT(KWTD) = -1*(ZWT-ZSOIL(IWTD))
       END IF
         ...
         DO IZ = 1, MIN(MIN(KROOT+1,NSOIL), KWTD)
         ...
         ! Calculate PSI
           PSIZ(IZ) = MAX(PSIWLT,-parameters%PSISAT(IZ)*(SH2O(IZ)/parameters%SMCMAX(IZ))**(-parameters%BEXP(IZ)) )
         ...
         IF(TOTEASY .eq. 0.)THEN
           ROOTACTIVITY = 0.0
           !TML: correct BTRAN to be consistent with ROOTACTIVITY
           BTRAN = MPE
...
! soil surface resistance for ground evap.
     ...
     ELSE
       ...
       IF(OPT_RSF == 1 .OR. OPT_RSF == 4) THEN
         ...
         L_RSURF_PET = (-ZSOIL(1)) * ( exp ( (1.0 - MIN(1.0,parameters%SMCMAX(1)/parameters%SMCMAX(1))) ** 5 ) - 1.0 ) / ( 2.71828 - 1.0 ) 
         D_RSURF_PET = 2.2E-5 * parameters%SMCMAX(1) * parameters%SMCMAX(1) * ( 1.0 - parameters%SMCWLT(1) / parameters%SMCMAX(1) ) ** (2.0+3.0/parameters%BEXP(1))
         RSURF_PET = L_RSURF_PET / D_RSURF_PET
       ...
       PSI_PET   = -parameters%PSISAT(1)*(MAX(0.01,parameters%SMCMAX(1))/parameters%SMCMAX(1))**(-parameters%BEXP(1))   
       RHSUR_PET = FSNO + (1.-FSNO) * EXP(PSI_PET*GRAV/(RW*TG)) 
     END IF
...
!jref:end                            
        ...
        ! PET code from Sujay 
        CALL VEGE_FLUX_4PET (parameters,NSNOW   ,NSOIL   ,ISNOW   ,VEGTYP  ,VEG     , & !in
                    DT      ,SAV     ,SAG     ,LWDN    ,UR      , & !in
                    UU      ,VV      ,SFCTMP  ,THAIR   ,QAIR    , & !in
                    EAIR    ,RHOAIR  ,SNOWH   ,VAI     ,GAMMAV   ,GAMMAG   , & !in
                    FWET    ,LAISUN  ,LAISHA  ,CWP     ,DZSNSO  , & !in
                    ...
                    Z0MG    ,EMV     ,EMG     ,CANLIQ  ,FSNO, & !in
                    CANICE  ,STC     ,DF      ,RSSUN   ,RSSHA   , & !in
                    RSURF_PET   ,LATHEAV ,LATHEAG ,PARSUN  ,PARSHA  ,IGS     , & !in
                    FOLN    ,CO2AIR  ,O2AIR   ,BTRAN   ,SFCPRS  , & !in
                    RHSUR_PET   ,ILOC    ,JLOC    ,Q2      ,PAHV  ,PAHG  , & !in
                    EAH_T   ,TAH_T   ,TV_T    ,TGV_T   ,CMV_T   , & !inout
                    CHV_T   ,DX_T    ,DZ8W_T  ,                   & !inout
                    TAUXV_T ,TAUYV_T ,IRG_T   ,IRC_T   ,SHG_T   , & !out
                    SHC_T   ,EVG_T   ,EVC_T   ,TR_T    ,GHV_T   , & !out
                    T2MV_T  ,PSNSUN_T,PSNSHA_T ,                   & !out
!jref:start
                    QC      ,QSFC    ,PSFC    , & !in
                    Q2V     ,CHV2, CHLEAF, CHUC, &
...
!energy balance at bare ground: SAG*(1.-FVEG)=(IRB+SHB+EVB+GHB)*(1.-FVEG) at 1-FVEG
	...
	Z0WRF = Z0M
        ...
        FGEV_PET = FVEG * EVG_T   + (1.0 - FVEG) * EVB
        FCEV_PET  = EVC_T
        FCTR_PET  = TR_T
...
! update CH for output
  ...
  ! PET code from Sujay 
  SUBROUTINE VEGE_FLUX_4PET(parameters,NSNOW   ,NSOIL   ,ISNOW   ,VEGTYP  ,VEG     , & !in
                       DT      ,SAV     ,SAG     ,LWDN    ,UR      , & !in
                       UU      ,VV      ,SFCTMP  ,THAIR   ,QAIR    , & !in
                       EAIR    ,RHOAIR  ,SNOWH   ,VAI     ,GAMMAV   ,GAMMAG,  & !in
                       FWET    ,LAISUN  ,LAISHA  ,CWP     ,DZSNSO  , & !in
     ...
     IF(ITER == 1) THEN
        IF (OPT_CRS == 1) then  ! Ball-Berry
         ...
         CALL STOMATA (parameters,VEGTYP,MPE   ,PARSHA ,FOLN  ,ILOC  , JLOC , & !in
                       ...
                       O2AIR ,CO2AIR,IGS    ,BTRAN ,RB    , & !in
                       RSSHA ,PSNSHA)                         !out
         !SET canopy resistance to RSMIN
         RSSUN = parameters%RSMIN
         RSSHA = parameters%RSMIN
...
CONTAINS
  ! modified by Shugong Wang  11/06/2018
  !subroutine read_mp_veg_parameters(DATASET_IDENTIFIER)
   subroutine read_mp_veg_parameters(DATASET_IDENTIFIER, mp_table_file)

    implicit none
    character(len=*), intent(in) :: DATASET_IDENTIFIER
    character(len=*), intent(in) :: mp_table_file 
    integer :: ierr
    INTEGER :: IK,IM
    ...
    NAMELIST / noahmp_umd_veg_categories / VEG_DATASET_DESCRIPTION, NVEG
    ...
    open(15, file=trim(mp_table_file), status='old', form='formatted', action='read', iostat=ierr)
    if (ierr /= 0) then
       write(*,'("****** Error ******************************************************")')
    ...
    if ( trim(DATASET_IDENTIFIER) == "USGS" ) then
       ...
       read(15,noahmp_umd_veg_categories)
       read(15,noahmp_umd_parameters)
    else
  ...
  ! modified by Shugong Wang 11/05/2018
  !subroutine read_mp_soil_parameters()
  subroutine read_mp_soil_parameters(soil_tabel, gen_tabel)
    IMPLICIT NONE
    character(len=*) :: soil_tabel
    character(len=*) :: gen_tabel
    INTEGER :: IERR
    CHARACTER*4         :: SLTYPE
...
!
    OPEN(19, FILE=trim(soil_tabel),FORM='FORMATTED',STATUS='OLD',IOSTAT=ierr)
    IF(ierr .NE. 0 ) THEN
...
!
    OPEN(19, FILE=trim(gen_tabel),FORM='FORMATTED',STATUS='OLD',IOSTAT=ierr)
    IF(ierr .NE. 0 ) THEN
  ...
  subroutine read_mp_rad_parameters(mp_table_file)
    implicit none
    character(len=*) :: mp_table_file 
    integer :: ierr
  ...
  subroutine read_mp_global_parameters(mp_table_file)
    implicit none
    character(len=*) :: mp_table_file
    integer :: ierr
...
RSURF_SNOW_TABLE     = RSURF_SNOW
  ...
  subroutine read_mp_crop_parameters(mp_table_file)
    implicit none
    character(len=*) :: mp_table_file 
    integer :: ierr
 ...
 DEFAULT_CROP_TABLE      = DEFAULT_CROP
  ...
  subroutine read_mp_optional_parameters(mp_table_file)
    implicit none
    character(len=*) :: mp_table_file 
    integer :: ierr
